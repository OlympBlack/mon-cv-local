            ...
            <div className="flex flex-grow bg-slate-50 dark:bg-slate-900">
                        ...
                        <Section title="Formations" color={data.color}>
                            <div className="space-y-6 border-l-2 border-slate-200 dark:border-slate-700 ml-3 pl-8 py-2">
                                {data.education.map((edu, i) => (
                                    <div key={i} className="relative">
                                        <span className="absolute -left-[37px] top-1.5 w-3 h-3 rounded-full bg-slate-400 border-2 border-white dark:border-slate-600" />
                ...
                <aside className="w-1/3 p-10 bg-slate-100 dark:bg-slate-800 border-l border-slate-200 dark:border-slate-700">
                        ...
                        <Section title="Outils" small color={data.color}>
                            <div className="flex flex-wrap gap-2">
                                {data.tools.map((tool, i) => (
                                    <span key={i} className="bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 px-2 py-1 rounded text-xs font-semibold text-slate-600 dark:text-gray-300">{tool}</span>
                                ))}
                            </div>
                        </Section>
                    )}
                            ...
                            <div className="space-y-3">
                                {data.certifications.map((cert, i) => (
                                    <div key={i} className="text-sm">
                                        <p className="font-bold text-slate-700 dark:text-gray-200">{cert.name}</p>
                                        <p className="text-xs text-slate-500 dark:text-gray-400">{cert.issuer}, {cert.year}</p>
                                    </div>
                                ))}
                            </div>
                        </Section>
                    ...
                    <Section title="Langues" small color={data.color}>
                        <ul className="space-y-2 text-sm font-medium text-slate-700 dark:text-gray-300">
                    ...
                    <Section title="Intérêts" small color={data.color}>
                        <div className="flex flex-wrap gap-2">
                            {data.hobbies.map((hobby, i) => (
                                <span key={i} className="bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 px-3 py-1 rounded text-xs font-semibold text-slate-600 dark:text-gray-300">
                                    {hobby}
                                </span>
                            ))}
                        </div>
