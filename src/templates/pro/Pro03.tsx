    ...
    return (
        <PdfSafeWrapper>
            <div className="w-[800px] min-h-[1100px] bg-white dark:bg-slate-900 shadow-lg font-sans text-gray-800 dark:text-gray-100 flex flex-col">
                ...
                {/* HEADER */}
                <div className="text-white p-12" style={{ backgroundColor: data.color, color: 'white' }}>
                ...
                <div className="flex-grow p-12 grid grid-cols-3 gap-12 bg-gray-50 dark:bg-slate-900">
                    ...
                    <div className="col-span-1 space-y-10">
                            ...
                            <div className="flex flex-col gap-3">
                                {data.skills.map((skill, i) => (
                                    <div key={i} className="bg-white dark:bg-slate-800 p-3 rounded shadow-sm flex items-center justify-between">
                                        ...
                                        <span className="text-xs font-bold text-white px-2 py-1 rounded" style={{ backgroundColor: data.color, color: 'white' }}>{skill.level}%</span>
                                    </div>
                                ))}
                            </div>
                        </section>
                            ...
                            <section>
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {data.tools.map((tool, i) => (
                                        <div key={i} className="p-2 bg-white dark:bg-slate-700 rounded shadow-sm border border-gray-100 dark:border-slate-600" title={tool.label}>
                                            <img
                        ...
                        {data.links && data.links.length > 0 && (
                            <section>
                                <h3 className="section-title border-b-2 pb-2 mb-4 font-bold uppercase text-gray-700 dark:text-gray-200" style={{ borderColor: data.color }}>Ressources</h3>
                                <div className="bg-white dark:bg-slate-800 p-4 rounded shadow-sm">
                                    <ul className="space-y-2 text-sm">
                                        {data.links.map((link, i) => (
                                            <li key={i}>
                                                <a href={link.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:underline text-gray-700 dark:text-gray-300">
