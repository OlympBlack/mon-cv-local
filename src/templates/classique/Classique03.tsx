...
export default function CVTemplate({ data }: CVTemplateProps) {
    const primaryColor = data.color || '#6b46c1'; // Purple default
    return (
        <PdfSafeWrapper>
            <div className="w-[800px] min-h-[1100px] bg-white dark:bg-slate-900 shadow-lg flex flex-col font-sans text-gray-800 dark:text-gray-100">
                {/* HEADER */}
                <header className="text-white p-10 flex justify-between items-center" style={{ backgroundColor: primaryColor, color: 'white' }}>
                ...
                <div className="flex flex-grow bg-slate-50 dark:bg-slate-900">
                    <main className="w-2/3 p-10 bg-white dark:bg-slate-900">
                        <Section title="Profil Professionnel" color={primaryColor}>
                        ...
                        <Section title="Expérience" color={primaryColor}>
                            <div className="space-y-8 border-l-2 border-slate-200 dark:border-slate-700 ml-3 pl-8 py-2">
                                {data.experiences.map((exp, i) => (
                                    <div key={i} className="relative">
                                        <span className="absolute -left-[41px] top-1 w-5 h-5 rounded-full border-4 border-white shadow-sm" style={{ backgroundColor: primaryColor }} />
                                        <h3 className="font-bold text-lg text-slate-800 dark:text-gray-100">{exp.role}</h3>
                                        <div className="font-semibold text-sm mb-2" style={{ color: primaryColor }}>{exp.company} | {formatDateRange(exp.startDate, exp.endDate, exp.isCurrent)}</div>
                                        {exp.description && (
                        ...
                        {data.education && data.education.length > 0 && (
                            <Section title="Formations" color={primaryColor}>
                    ...
                    <aside className="w-1/3 p-10 bg-slate-100 dark:bg-slate-800 border-l border-slate-200 dark:border-slate-700">
                        <Section title="Compétences" small color={primaryColor}>
                                ...
                                {data.skills.map((skill, i) => (
                                    <div key={i}>
                                        ...
                                        <div className="h-2 bg-slate-300 dark:bg-slate-600 rounded-full overflow-hidden">
                                            <div className="h-full" style={{ width: `${skill.level}%`, backgroundColor: primaryColor }} />
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        {data.tools && data.tools.length > 0 && (
                            <Section title="Outils" small color={primaryColor}>
                                <div className="flex flex-wrap gap-2">
                                    {data.tools.map((tool, i) => (
                                        <div key={i} className="bg-slate-200 dark:bg-slate-700 p-1.5 rounded" title={tool.label}>
                                            <img
                        ...
                        {data.links && data.links.length > 0 && (
                            <Section title="Ressources" small color={primaryColor}>
                                <ul className="space-y-1">
                                    {data.links.map((link, i) => (
                                        <li key={i}>
                                            <a href={link.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-gray-700 dark:text-gray-300 hover:text-black dark:hover:text-white hover:underline text-sm">
                        ...
                        {data.certifications && data.certifications.length > 0 && (
                            <Section title="Certificats" small color={primaryColor}>
                        ...
                        <Section title="Langues" small color={primaryColor}>
                        ...
                        <Section title="Intérêts" small color={primaryColor}>
                            <div className="flex flex-wrap gap-2">
